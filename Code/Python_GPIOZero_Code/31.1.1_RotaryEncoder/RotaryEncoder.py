...
def rotaryDeal():
    ...
    if symbol == 1:
        symbol = 0
        if (lastDTStatus == 1) and (
            currentDTStatus == 0
        ):  # Rotate clockwise, the angular displacement increases, and the count value increases
            previousCounterValue = previousCounterValue + 1
        if (lastDTStatus == 0) and (
            currentDTStatus == 1
        ):  # Counterclockwise rotation, angular displacement reduction, count value reduction
            previousCounterValue = previousCounterValue - 1
